...
if [ -e "$THE_PATH" ]; then
	...
	if [ -e venv/bin/python ] && test $UBUNTU_DEBIAN; then
		echo "Installing vdf from source on Ubuntu/Debian"
		...
		echo "Installing vdf from source on RedHat/CentOS/Fedora"
		# Install remaining needed development tools - assumes venv and prior run of install.sh
		...
		echo "Installing vdf requirement boost for MacOS."
		brew install boost
		echo "Installing vdf from source."
