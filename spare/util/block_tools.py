...
from spare.cmds.init_funcs import create_all_ssl, create_default_spare_config
from spare.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from spare.plotting.create_plots import create_plots
from spare.consensus.block_creation import create_unfinished_block, unfinished_block_to_full_block
...
from spare.plotting.plot_tools import PlotInfo, load_plots, parse_plot_info
...
from spare.types.generator_types import BlockGenerator, CompressorArg
...
class BlockTools:
    ...
    def __init__(
        ...
        if len(self.pool_pubkeys) == 0 or len(farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `spare generate keys`")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("SPARE_ROOT", "~/.spare/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
