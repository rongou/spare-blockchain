...
import { mojo_to_colouredcoin_string, mojo_to_spare_string } from '../../util/spare';
...
const getCols = (type: WalletType) => [
  ...
  },
  {
    field: (row: Row) => type === WalletType.COLOURED_COIN
      ...
      : mojo_to_spare_string(row.amount),
    title: <Trans>Amount</Trans>,
  },
  {
    field: (row: Row) => mojo_to_spare_string(row.fee_amount),
...
export default function WalletHistory(props: Props) {
  ...
  return (
    <Card
      title={  
      <Typography >
        <span style={ { color: "#E9398D", fontSize: 24, fontWeight:400, fontFamily:"Josefin" }}><Trans>History</Trans></span>
      </Typography>
      }
    >
