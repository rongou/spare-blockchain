...
import { mojo_to_spare_string, spare_to_mojo } from '../../../util/spare';
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const spareper_input = ip_parsed.limit;
    ...
    const spareper_value = Number.parseInt(Number(spareper_input));
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_spare_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = spare_to_mojo(amount_input.value);
    const fee = spare_to_mojo(fee_input.value);
