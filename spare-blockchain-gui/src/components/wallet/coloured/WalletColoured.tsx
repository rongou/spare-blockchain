...
import { AlertDialog, Card, Flex } from '@spare/core';
...
import {
  colouredcoin_to_mojo, mojo_to_colouredcoin_string
} from '../../../util/spare';
...
function BalanceCard(props: BalanceCardProps) {
  ...
  const balance_ptotal_spare = mojo_to_colouredcoin_string(balance_ptotal);
  const balance_pending_spare = mojo_to_colouredcoin_string(balance_pending);
  const balance_change_spare = mojo_to_colouredcoin_string(balance_change);
  const acc_content =
    balancebox_1 +
    balancebox_2 +
    balancebox_ptotal +
    balancebox_3 +
    balance_ptotal_spare +
    balancebox_unit +
    balancebox_hline +
    balancebox_4 +
    balancebox_row +
...
function SendCard(props: SendCardProps) {
  ...
  function send() {
    ...
    if (address.includes('spare_addr') || address.includes('colour_desc')) {
      dispatch(
        openDialog(
          <AlertDialog>
